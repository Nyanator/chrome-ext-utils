...
/** チャンネルデータの型 */
export type ChannelData<T extends ChannelMap, K extends keyof T> = Readonly<T[K]["data"]>;

/** チャンネル応答の型 */
export type ChannelResponse<T extends ChannelMap, K extends keyof T> = T[K]["response"];
  ...
  implements ChannelListenerMap<T>
{
  ...
  private readonly listeners: Map<keyof T, ChannelListener<T, keyof T>[]> = new Map();
