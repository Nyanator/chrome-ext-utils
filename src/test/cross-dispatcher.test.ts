...
describe("CrossDispatcherクラス", () => {
  ...
  it("strictModeが無効な場合、再起呼び出しの際に空の配列を返す", async () => {
    const dispatcher = CrossDispatcher<TestChannelMap>({ strictMode: false });
    ...
    dispatcher.channel({
      ...
      channel3: async () => {
        await expect(
          dispatcher.dispatch({ channelKey: "channel1", channelData: 3 }),
        ).toBe([]);
  ...
  it("strictModeが有効な場合、再起呼び出しの際に例外が発生する", async () => {
    const dispatcher = CrossDispatcher<TestChannelMap>({ strictMode: true });
