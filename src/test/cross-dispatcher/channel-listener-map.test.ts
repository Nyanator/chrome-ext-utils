import { ChanneListenerMapImpl } from "../../cross-dispatcher/channel-listener-map-impl";
import { createChannelListenerMap } from "../../cross-dispatcher/factories";

import { TestChannelMap, TestInterface, TestType } from "./mock-interface";

describe("ChannelListenerMapImpl クラス", () => {
  it("正しくリスナーを登録できる", async () => {
    const map = createChannelListenerMap();
  ...
  it("リスナーの型推論が働いているか", async () => {
    const map = createChannelListenerMap<TestChannelMap>();
    // 型推論が働けばコンパイルが通る
    map.channel({
