import { DefaultCrossDispatcher } from "../../cross-dispatcher/default-cross-dispathcher";
import { createCrossDispatcher } from "../../cross-dispatcher/factories";

import { TestChannelMap, TestInterface, TestType } from "./mock-interface";

describe("DefaultCrossDispatcherクラス", () => {
  it("正しくリスナーを登録できる", async () => {
    const dispatcher = createCrossDispatcher<TestChannelMap>();
    const channelData = 1;
    ...
    // カバレージのために呼び出しのテスト
    dispatcher.getAllChannelListeners();
  });

  it("複数のチャンネルキーをディスパッチできる", async () => {
    const dispatcher = createCrossDispatcher();
    const channelData = "1";
  ...
  it("strictModeが無効な場合、再起呼び出しの際に空の配列を返す", async () => {
    const dispatcher = createCrossDispatcher<TestChannelMap>(false);
    ...
    dispatcher.channel({
      ...
      channel3: async () => {
        await expect(dispatcher.dispatch("channel1", 3)).toBe([]);
  ...
  it("strictModeが有効な場合、再起呼び出しの際に例外が発生する", async () => {
    const dispatcher = createCrossDispatcher<TestChannelMap>(true);
  ...
  it("strictModeが有効な場合、存在しないイベントのディスパッチには例外をスローする", async () => {
    const dispatcher = createCrossDispatcher(true);

    await expect(
      dispatcher.dispatch("nonExistentchannel", {}),
  ...
  it("非同期リスナーが何故かError以外をスローしたとき 不正なスローを例外で報告する", async () => {
    const dispatcher = createCrossDispatcher();
    dispatcher.channel({
      channel1: async () => {
        throw 1;
  ...
  it("同期的なリスナーが何故かError以外をスローしたとき 不正なスローを例外で報告する", async () => {
    const dispatcher = createCrossDispatcher();
    dispatcher.channel({
      channel1: () => {
        throw 1;
  ...
  it("リスナーを正しく解除できる", () => {
    ...
    dispatcher.removeListener("channel1", channelListener);
  ...
  it("チャンネルキーに紐づくリスナーを全て解除できる", () => {
    ...
    dispatcher.removeAllListenersForChannel("channel1");
    dispatcher.dispatch("channel1", {});
    expect(listenerCalled).toBe(false);
  });
