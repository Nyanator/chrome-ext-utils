...
describe("initializeDIContainer関数の結合テスト", () => {
  ...
  it("DIコンテナにChannelListenerMapが正しく登録されていることを確認する", () => {
    const instance =
      container.resolve<ChanneListenerMapImpl<ChannelMap>>("ChannelListenerMap");
    expect(instance).toBeInstanceOf(ChanneListenerMapImpl);
  });

  it("DIコンテナにCrossDispatcherがシングルトンとして正しく登録されていることを確認する", () => {
    const instance1 =
      container.resolve<CrossDispatcherImpl<ChannelMap>>("CrossDispatcher");
    const instance2 =
      container.resolve<CrossDispatcherImpl<ChannelMap>>("CrossDispatcher");
  ...
  it("DIコンテナにMessageValidatorConfigが正しく登録されていることを確認する", () => {
    const config = container.resolve<MessageValidatorConfig>("MessageValidatorConfig");
  ...
  it("DIコンテナにSessionStaticTokenが正しく登録されていることを確認する", () => {
    const instance = container.resolve<SessionStaticToken>("SessionStaticToken");
    expect(instance).toBeInstanceOf(SessionStaticToken);
  });

  it("DIコンテナにSessionStaticKeyが正しく登録されていることを確認する", () => {
    const instance = container.resolve<SessionStaticKey>("SessionStaticKey");
