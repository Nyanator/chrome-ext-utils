...
describe("DOM操作ユーティリティ", () => {
  ...
  it("appendScriptText 要素にスクリプト要素を追加する", () => {
    ...
    expect(element.querySelector("script")?.textContent).toBe('console.log("test");');
  });

  it("appendStyleTextToHead ヘッド要素にスタイル要素を追加する", () => {
    const element = document.createElement("div");
    DomUtils.appendStyleTextToHead(element, ".test { color: red; }");
    expect(element.querySelector("style")?.textContent).toBe(".test { color: red; }");
  });
  ...
  it("isInView 要素が画面内に表示されているかどうかを判定する", () => {
    ...
    expect(
      DomUtils.isInView(mockWindow as Window, mockElement as unknown as Element),
    ).toBe(true);
